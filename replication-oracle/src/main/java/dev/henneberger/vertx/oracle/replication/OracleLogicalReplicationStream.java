...
public class OracleLogicalReplicationStream extends AbstractJdbcPollingReplicationStream<OracleChangeEvent> {
  ...
  protected void logStreamFailure(Throwable error) {
    LOG.error("Oracle CDC stream failed for {}", options.getSourceTable(), error);
  }
  ...
  @Override
  protected OracleChangeEvent mapRow(ResultSet rs) throws Exception {
  ...
  @Override
  protected String eventPosition(OracleChangeEvent event) {
    return event.getPosition();
  }

  @Override
  protected String checkpointKey() {
    return "oracle:" + options.getDatabase() + ":" + options.getSourceTable();
  }
  ...
  private static OracleChangeEvent.Operation mapOperation(String op) {
    ...
    if (n.startsWith("INS")) {
      return OracleChangeEvent.Operation.INSERT;
    }
    if (n.startsWith("DEL")) {
      return OracleChangeEvent.Operation.DELETE;
    }
