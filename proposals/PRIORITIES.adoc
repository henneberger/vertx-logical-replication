...
- Adopted contract tests for `replication-oracle`, `replication-db2`, `replication-mariadb`, and `replication-cockroachdb`.

== P2 (In Progress)

1. Option validation unification
- Added shared `OptionValidation` helper in `replication-core`.
- Migrated validation logic for `replication-oracle`, `replication-db2`, `replication-mariadb`, and `replication-cockroachdb` options.
- Remaining modules still using local validation helpers will be migrated incrementally.
