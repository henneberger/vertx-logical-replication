...
- Added `AbstractWorkerReplicationStream` in `replication-core`.
...
- Migrated `replication-cassandra` and `replication-scylladb` logical streams to the shared worker runtime.
- Migrated `replication-mongodb` and `replication-mysql` logical streams to the shared worker runtime.
- Migrated `replication-sqlserver` and `replication-neo4j` logical streams to the shared worker runtime.
- Remaining shared-runtime rollout target: `replication-postgres` (protocol-specific stream path).
