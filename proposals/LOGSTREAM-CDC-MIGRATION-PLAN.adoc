...
- Shared JDBC polling runtime extracted to `replication-core` and adopted by Oracle/Db2/MariaDB/CockroachDB adapters.
- Added typed preflight failure exception (`PreflightFailedException`) carrying report/state across adapters.
- SQL Server events now enrich `commitTimestamp` via `sys.fn_cdc_map_lsn_to_time(__$start_lsn)` when available.
- Added shared `ReplicationStreamContractKit` and adopted contract tests for Oracle/Db2/MariaDB/CockroachDB adapters.
